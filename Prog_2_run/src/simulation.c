...
void simulation(struct Metrics* metrics)
{
	...
	FILE* f_sys_resp = malloc(sizeof(intptr_t));
	FILE* f_sys_thr = malloc(sizeof(intptr_t));
	FILE* f_fs_resp = malloc(sizeof(intptr_t));
	FILE* f_fs_thr = malloc(sizeof(intptr_t));
	FILE* f_be_resp = malloc(sizeof(intptr_t));
	FILE* f_be_delay = malloc(sizeof(intptr_t));
	FILE* f_be_thr = malloc(sizeof(intptr_t));
	FILE* f_drop_ratio = malloc(sizeof(intptr_t));
	FILE* f_abort_ratio = malloc(sizeof(intptr_t));
	...
	while(c->events_times[NEW_SESSION_INDEX] < STOP_SIMULATION || xfs + xbe + session_th > 0)
	{
		...
		switch (index) {
			...
			case SAMPLING_INDEX:
				...
				//FILE CHE CONTERRANNO L'ANDAMENTO TEMPORALE DELLE METRICHE PER OGNI RUN:
				f_sys_resp = fopen("system_response_time","a+");
				f_sys_thr = fopen("system_throughput","a+");
				f_fs_resp = fopen("fs_response_time","a+");
				f_fs_thr = fopen("fs_throughput","a+");
				f_be_resp = fopen("be_response_time","a+");
				f_be_delay = fopen("be_delay_time","a+");
				f_be_thr = fopen("be_throughput","a+");
				f_drop_ratio = fopen("drop_ratio","a+");
				f_abort_ratio = fopen("abort_ratio","a+");

				fprintf(f_sys_resp,"%6.6f , ",metrics->sys_resp);
				fprintf(f_sys_thr,"%6.6f , ",metrics->sys_thr);
				fprintf(f_fs_resp,"%6.6f , ",metrics->fs_resp);
				fprintf(f_fs_thr,"%6.6f , ",metrics->fs_thr);
				fprintf(f_be_resp,"%6.6f , ",metrics->be_resp);
				fprintf(f_be_delay,"%6.6f , ",metrics->be_delay);
				fprintf(f_be_thr,"%6.6f , ",metrics->be_thr);
				fprintf(f_drop_ratio,"%6.6f , ",metrics->drop_ratio);
				fprintf(f_abort_ratio,"%6.6f , ",metrics->abort_ratio);
	...
	fprintf(f_be_delay,"\n\n");
	fprintf(f_be_thr,"\n\n");
	fprintf(f_drop_ratio,"\n\n");
	fprintf(f_abort_ratio,"\n\n");
