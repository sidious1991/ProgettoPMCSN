...
struct Metrics{
	...
	double fs_util; // Area
	double fs_pop; // Popolazione del FS
	...
	double th_pop; // Popolazione del nodo di Think
	...
	double total_time; // Tempo totale di esecuzione della simulazione (può essere più di 10000)
};
...
void simulation(struct Metrics* measures)
{
	...
	while(c->events_times[NEW_SESSION_INDEX] < STOP_SIMULATION || xfs + xbe + session_th > 0)
	{
		...
		metrics->be_thr = (num_ended_req_be)/(cl->current); // Throughput be
		...
		//Modifica delle measures -->> PER INTERVALLI DI CONFIDENZA A 10.000 SECONDI (TAGLIO DELLA SIMULAZIONE)

		if(cl->current == STOP_SIMULATION)
		{
			measures->sys_resp = metrics->sys_resp;
			measures->sys_thr = metrics->sys_thr;
			measures->fs_resp = metrics->fs_resp;
			measures->fs_thr = metrics->fs_thr;
			measures->fs_pop = metrics->fs_pop;
			measures->fs_util = metrics->fs_util;
			measures->be_resp = metrics->be_resp;
			measures->be_delay = metrics->be_delay;
			measures->be_thr = metrics->be_thr;
			measures->th_pop = metrics->th_pop;
			measures->drop_ratio = metrics->drop_ratio;
			measures->abort_ratio = metrics->abort_ratio;
		}

		//FINE Modifica delle measures -->> PER INTERVALLI DI CONFIDENZA A 10.000 SECONDI (TAGLIO DELLA SIMULAZIONE)
		...
		switch (index) {
			...
			case NEW_SESSION_INDEX:
				...
				if(ab_dr == false)
				{
					...
					metrics->fs_pop = xfs + session_fifo_fs;
			...
			case SAMPLING_INDEX:
				...
				if(cl->current <= STOP_SIMULATION)
				{
						...
						else if (strcmp(choice,"fs_popolation") == 0)
							trend[(int)(cl->current - 1)] += (metrics->fs_pop);

						else if (strcmp(choice,"fs_util") == 0)
							trend[(int)(cl->current - 1)] += (metrics->fs_util);
						...
						else if (strcmp(choice,"th_popolation") == 0)
							trend[(int)(cl->current - 1)] += metrics->th_pop;
...
int main(void)
{
	...
	printf("\nENTER THE METRIC TO MEASURE (ITS TIME TREND) [ENTER]:\n1.sys_resp\n2.sys_thr\n3.fs_resp\n4.fs_thr\n5.fs_popolation\n6.5.fs_util\n7.be_resp\n8.be_delay\n9.be_thr\n10.th_popolation\n11.drop_ratio\n12.abort_ratio\n\n");
	...
	for(i = 1 ; i < 51 ; i++)
	{
		...
		var->fs_pop = var->fs_pop + pow((metrics->fs_pop - mean->fs_pop) , 2.0)*(i-1)/i;
		var->fs_util = var->fs_util + pow((metrics->fs_util - mean->fs_util) , 2.0)*(i-1)/i;
		...
		var->th_pop = var->th_pop + pow((metrics->th_pop - mean->th_pop) , 2.0)*(i-1)/i;
		...
		mean->fs_pop = (mean->fs_pop) + (metrics->fs_pop - mean->fs_pop)/i;
		mean->fs_util = (mean->fs_util) + (metrics->fs_util - mean->fs_util)/i;
		...
		mean->th_pop = (mean->th_pop) + (metrics->th_pop - mean->th_pop)/i;
	...
	printf("Mean: %6.6f\n",mean->sys_resp);
	printf("Var: %6.6f\n",(var->sys_resp/(i-2)));
	printf("Confidence Interval (level 0.95): (+/-)%6.6f\n\n", confidence_interval(i-1 , (var->sys_resp/(i-2))));
	...
	printf("Popolation FS: \n");
	printf("Mean: %6.6f\n",mean->fs_pop);
	printf("Var: %6.6f\n",(var->fs_pop/(i-2)));
	printf("Confidence Interval (level 0.95): (+/-)%6.6f\n\n", confidence_interval(i-1 , (var->fs_pop/(i-2))));

	printf("Utilization FS: \n");
	printf("Mean: %6.6f\n",mean->fs_util);
	printf("Var: %6.6f\n",(var->fs_util/(i-2)));
